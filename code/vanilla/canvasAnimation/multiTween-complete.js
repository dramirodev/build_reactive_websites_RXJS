...
function percentDone(durationSec) {
    return function ($obs) {
        ...
        return $obs.pipe(map(() => (animationFrame.now() - startTime) / (durationSec * 1000)));
    };
}
...
function finalTween(easingFn, element, endPoint, duration) {
    ...
    interval(1000 / 60, animationFrame)
        .pipe(percentDone(duration), takeWhile(percentDone => percentDone <= 1), easingMap(easingFn))
        .subscribe((movePercent) => {
