...
fromEvent(searchBar, 'keyup')
    .pipe(map(event => event.target.value), filter(query => query.length > 3), distinctUntilChanged(), debounceTime(333), tap(() => loadingEl.style.display = 'block'), switchMap(query => ajax(endpoint + query)), catchError((err, caught$) => merge(of({ err }), caught$)), tap(() => loadingEl.style.display = 'none'))
