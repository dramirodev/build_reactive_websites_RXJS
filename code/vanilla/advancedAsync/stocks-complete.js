...
let stockStream$ = webSocket(endpoint)
    .pipe(scan((accumulatedData, nextItem) => {
...
function makeCheckboxStream(checkboxEl, stockName) {
    return merge(fromEvent(checkboxEl, 'change'), 
    // Start off checked
    of({ target: { checked: true } }))
        .pipe(map(e => e.target.checked), map(isEnabled => ({
        isEnabled,
        stock: stockName
    })));
}
let settings$ = combineLatest(makeCheckboxStream(abcEl, 'ABC'), makeCheckboxStream(defEl, 'DEF'), makeCheckboxStream(ghiEl, 'GHI'), makeCheckboxStream(jklEl, 'JKL'), (...stockBoxes) => {
    return stockBoxes
        .filter(stockBox => stockBox.isEnabled)
        .map(stockBox => stockBox.stock);
});
combineLatest(settings$, stockStream$, (enabledStocks, stockUpdates) => ({ enabledStocks, stockUpdates }))
    .pipe(map(({ enabledStocks, stockUpdates }) => {
    return stockUpdates
