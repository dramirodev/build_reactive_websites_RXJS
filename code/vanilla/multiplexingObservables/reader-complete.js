...
let chatStream$ = webSocket(wsUrl);
let userSubject$ = new AsyncSubject();
function authenticateUser(username) {
    let user$ = ajax('http://localhost:3000/api/multiplexingObservables/chat/user/'
        + username)
        .pipe(map(data => data.response));
    user$.subscribe(userSubject$);
}
merge(fromEvent(loginBtn, 'click'), fromEvent(loginInput, 'keypress')
    .pipe(
// Ignore all keys except for enter
filter(e => e.keyCode === 13)))
    .pipe(map(() => loginInput.value), filter(Boolean), tap(showLoadingSpinner))
...
function makeRoomStream(roomName) {
    ...
    chatStream$
        .pipe(filter(msg => msg.room.name === roomName))
...
roomLoads$
    .pipe(tap(setActiveRoom), switchMap(room => roomStreams[room]))
    .subscribe(writeMessageToPage);
// ===== Sending Messages =====
merge(fromEvent(sendBtn, 'click'), fromEvent(msgBox, 'keypress')
    .pipe(
// Only emit event when enter key is pressed
filter(e => e.keyCode === 13)))
    .pipe(map(() => msgBox.value), filter(Boolean), tap(() => msgBox.value = ''), withLatestFrom(roomLoads$, userSubject$, (message, room, user) => ({ message, room, user })))
    .subscribe(val => chatStream$.next(val));
// ===== Unread =====
merge(chatStream$.pipe(map(msg => ({ type: 'message', room: msg.room.name }))), roomLoads$.pipe(map(room => ({ type: 'roomLoad', room: room }))))
    .pipe(scan((unread, event) => {
    // new message in room
    ...
    rooms: {},
    activeRoom: ''
}), map(unread => unread.rooms), map(rooms => Object.keys(rooms)
    .map(key => ({
    roomName: key,
    number: rooms[key]
}))))
